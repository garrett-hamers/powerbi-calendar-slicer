...
3. Select `dist/calendarSlicerATLYN606CC6AF684C4BBA.1.0.0.3.pbiviz`
...
1. **Build.** `npm run certify` (or `npm run package`) writes
   `dist/calendarSlicerATLYN606CC6AF684C4BBA.1.0.0.3.pbiviz` — `pbiviz` always
...
2. **Rename for the storefront.** Copy it to **`atlynCalendarSlicer.pbiviz`** to
   ...
   path `visuals/calendar-slicer/1.0.0.3/atlynCalendarSlicer.pbiviz`. Renaming
...
Keep the version pinned at `1.0.0.3` across `package.json`, `package-lock.json`,
`pbiviz.json`, and the
